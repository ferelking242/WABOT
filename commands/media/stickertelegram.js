...
async function stickerTelegramCommand(sock, chatId, msg) {
    try {
        ...
        // Using environment bot token for security
        const botToken = process.env.TELEGRAM_BOT_TOKEN || '7801479976:AAGuPL0a7kXXBYz6XUSR_ll2SR5V_W6oHl4';
        ...
        try {
            ...
            for (let i = 0; i < stickerSet.result.stickers.length; i++) {
                try {
                    ...
                    // Generate temp file paths using centralized API
                    const tempInput = temp.createTempFile(imageBuffer, { 
                        prefix: `tg_input_${i}` 
                    });
