...
async function emojimixCommand(sock, chatId, msg) {
    try {
        ...
        if (!args[0]) {
            await sock.sendMessage(chatId, { text: i18n.t(senderId, 'fun.emojimix_example') });
            return;
        }

        if (!text.includes('+')) {
            await sock.sendMessage(chatId, { 
                text: i18n.t(senderId, 'fun.emojimix_separator')
            });
            return;
        }
        ...
        if (!data.results || data.results.length === 0) {
            await sock.sendMessage(chatId, { 
                text: i18n.t(senderId, 'fun.emojimix_cannot_mix')
            });
            return;
        }
