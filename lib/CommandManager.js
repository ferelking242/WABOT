...
class CommandManager {
    ...
    detectPermissionLevel(category, commandName) {
        // Commandes owner (spécifiques uniquement)
        const ownerCommands = ['sudo', 'setpp', 'clearsession', 'autostatus', 'autodelete',
                              'autoread', 'autotyping', 'cleartmp', 'pair'];
        ...
        // Commandes user (par défaut) — inclut toutes les commandes système publiques
        return 'user';
    }
    ...
    async canExecuteCommand(groupId, commandName, userRole = 'user') {
        // 1. Vérifier si la commande existe — si introuvable, laisser passer (switch statement gère)
        const commandMeta = this.commandsCache.get(commandName);
        if (!commandMeta) {
            return { allowed: true, reason: 'COMMAND_NOT_IN_CACHE' };
        }
