...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    const botId = (typeof sock.user.id === 'string' ? sock.user.id : String(sock.user.id)).split(':')[0] + '@s.whatsapp.net';
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `*@${(typeof senderId === 'string' ? senderId : String(senderId)).split('@')[0]} bad words are not allowed here*`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `*@${(typeof senderId === 'string' ? senderId : String(senderId)).split('@')[0]} has been kicked for using bad words*`,
                    mentions: [senderId]
                });
            } catch (error) {
        ...
        case 'warn':
            ...
            if (warningCount >= 3) {
                try {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `*@${(typeof senderId === 'string' ? senderId : String(senderId)).split('@')[0]} has been kicked after 3 warnings*`,
                        mentions: [senderId]
                    });
                } catch (error) {
                ...
                await sock.sendMessage(chatId, {
                    text: `*@${(typeof senderId === 'string' ? senderId : String(senderId)).split('@')[0]} warning ${warningCount}/3 for using bad words*`,
                    mentions: [senderId]
                });
            }
            break;
