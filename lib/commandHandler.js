...
function buildMessageHandler(options) {
    ...
    return async function handleMessages(sock, messageUpdate, printLog) {
        ...
        try {
            ...
            // senderParticipantId = JID original (@lid dans les groupes Baileys v7 LID addressing)
            // utilisé uniquement pour la comparaison avec la liste des participants du groupe
            const senderParticipantId = message.key.participantAlt || message.key.participant || message.key.remoteJid;
            // senderId = JID téléphone @s.whatsapp.net (participantAlt) — utilisé pour toutes les vérifications d'identité
            // participantAlt est fourni par Baileys v7 pour résoudre le @lid vers le vrai numéro
