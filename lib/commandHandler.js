...
function buildMessageHandler(options) {
    ...
    return async function handleMessages(sock, messageUpdate, printLog) {
        ...
        try {
            ...
            // Accepter les types notify (reçus), append (envoyés par le bot), history (sync)
            ...
            // ── Ignorer les messages système/stub de Baileys ─────────────────
            // messageStubType = messages créés par Baileys pour les erreurs internes
            if (msg.messageStubType != null) return;

            // Ignorer les messages dont le contenu est uniquement technique (clés Signal,
            // protocole, distribution SKDM...) — jamais du texte traitable
            {
                const _keys = Object.keys(msg.message);
                const _nonText = ['senderKeyDistributionMessage', 'messageContextInfo',
                                  'protocolMessage', 'reactionMessage'];
                if (_keys.length > 0 && _keys.every(k => _nonText.includes(k))) return;
            }

            // Allow fromMe messages so the bot owner can use commands from their own number
            // Only skip if it's a protocol/system message the bot generated itself
            if (msg.key.fromMe && msg.message?.protocolMessage) return;
            ...
            // senderParticipantId = JID original @lid (Baileys v7 LID addressing)
            // Utilisé UNIQUEMENT pour comparer avec la liste des participants du groupe (qui contient aussi des @lid)
            const senderParticipantId = message.key.participant || message.key.remoteJid;
            // senderId = vrai JID téléphone @s.whatsapp.net
            // participantAlt = champ Baileys v7 qui contient le vrai numéro résolu depuis le @lid
            const senderId = message.key.participantAlt || senderParticipantId;
            const isGroup = chatId.endsWith('@g.us');
            const senderIsSudo = await isOwner(senderId, sock, chatId);
            ...
            if (userMessage.startsWith(prefix)) {
                ...
                try {
                    ...
                    // Continuer avec la commande originale si i18n échoue
                }
            }
            ...
            // Accept commands from both 'notify' (from others) and 'append' (owner self-messages)
            ...
            if (isGroup && isAdminCommand) {
                // Passer senderParticipantId (@lid) pour matcher la liste des participants du groupe
                const adminStatus = await isAdmin(sock, chatId, senderParticipantId, message);
            ...
            // Loguer silencieusement — ne JAMAIS envoyer "processing_error" au chat.
            // Ça spammait tous les groupes avec des erreurs internes (déchiffrement
            // Signal, protocolMessage, etc.) après chaque reconnexion.
            const code = error?.data || error?.output?.statusCode;
            // 429 → géré par circuit-breaker, pas besoin de logguer
            if (code === 429 || error?.message?.includes('429')) return;
            const msg = typeof error === 'object' ? (error?.message || JSON.stringify(error).slice(0, 120)) : String(error);
            console.warn(`[commandHandler/${botIdentity}] Erreur silencieuse: ${msg}`);
        }

        // Function to handle .jid command (internal utility)
        async function groupJidCommand(sock, chatId, message) {
            const userJid = message.key.participantAlt || message.key.participantAlt || message.key.participant || message.key.remoteJid;
            const chatJid = message.key.remoteJid;
            const senderId = message.key.participantAlt || message.key.participantAlt || message.key.participant || message.key.remoteJid;
