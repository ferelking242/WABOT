...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    ...
    try {
        // ÉTAPE 0: Vérification rapide via OWNER_LID (JIDs @lid pour WhatsApp multi-device)
        // OWNER_LID peut contenir plusieurs LIDs séparés par des virgules
        const ownerLidEnv = process.env.OWNER_LID;
        if (ownerLidEnv && senderId.endsWith('@lid')) {
            const senderBase = senderId.replace('@lid', '').split(':')[0];
            const ownerLids = ownerLidEnv.split(',').map(l => l.trim().replace('@lid', '').split(':')[0]);
            if (ownerLids.includes(senderBase)) {
                return true;
            }
        }

        // ÉTAPE 0b: Si c'est un @lid et qu'on a un sock, essayer de résoudre via contacts
        if (senderId.endsWith('@lid') && sock) {
            const contacts = sock.store?.contacts || sock.contacts || {};
            const contact = contacts[senderId];
            if (contact) {
                const phone = contact.lid ? contact.lid.split('@')[0].split(':')[0] : null;
                const ownerNumber = settings.ownerNumber;
                if (phone && phone === ownerNumber) {
                    return true;
                }
            }

            // Vérifier dans la DB si ce LID est enregistré comme propriétaire
            const user = await db.getUser(senderId);
            if (user && (user.is_owner || user.is_sudo)) {
                return true;
            }
            // Vérifier aussi sans le suffixe :device
            const senderBase = senderId.split(':')[0];
            if (senderBase !== senderId) {
                const userBase = await db.getUser(senderBase + '@lid');
                if (userBase && (userBase.is_owner || userBase.is_sudo)) {
                    return true;
                }
            }
        }
