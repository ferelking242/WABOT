...
exports.smsg = (XeonBotInc, m, store) => {
    ...
    if (m.key) {
        ...
        m.sender = XeonBotInc.decodeJid(m.fromMe && XeonBotInc.user.id || m.participant || m.key.participantAlt || m.key.participant || m.chat || '')
        if (m.isGroup) m.participant = XeonBotInc.decodeJid(m.key.participantAlt || m.key.participant) || ''
    }
    if (m.message) {
