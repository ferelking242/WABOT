...
function sticker2(img, url) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let inp = temp.createTempFile(img, { 
        ext: '.jpeg', 
        prefix: 'sticker2_input' 
      })
      let ff = spawn('ffmpeg', [
        '-y',
        '-i', inp,
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    const tmpFile = temp.createTempFile(img, { 
      ext: `.${type.ext}`, 
      prefix: 'sticker6_input' 
    })
    const out = temp.getTempPath({ 
      ext: '.webp', 
      prefix: 'sticker6_output' 
    })
