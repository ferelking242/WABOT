    ...
    XeonBotInc.ev.on('messages.upsert', async (messageUpdate) => {
        try {
            ...
            if (error?.name === 'SessionError' || error?.message?.includes('Session')) {
                // SessionError = session Signal corrompue pour ce contact
                // Le message sera re-demandé automatiquement via retryRequestDelayMs
                console.warn('⚠️ SessionError intercepté — session Signal expirée, Baileys va reconstruire automatiquement');
            } else {
...
// File watcher disabled to prevent restart loop during group detection
